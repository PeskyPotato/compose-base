```bash
mkdir config/ data/
docker compose up
```

Create a basic configuration file `./config/config.conf` like below.
```
[global]
    p: 8086, 3923   # Listen on ports 8086 and 3923
    e2dsa           # Enable EdDSA encryption for secure connections
    e2ts            # Enable TLS encryption
    z, qr           # Enable Zeroconf (LAN service discovery) and QR code login
    shr: /shr            # Shared folder path
    hist: /cfg/hists/    # History/log directory

[accounts]
  admin: admin

[/]
  /w
  accs:
    rwmda: admin

```

More examples can be found in [the documentation](https://github.com/9001/copyparty/tree/hovudstraum/scripts/docker#configuration).
